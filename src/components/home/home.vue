  ...
  <div class="main">
   ...
   <!-- SHOWING THE VIDEO LIST -->
    ...
    <v-card-actions>
     ...
     <v-btn class="ma-2" tile outlined color="primary">Preview
    </v-btn>
    <v-spacer></v-spacer>
    </v-card-actions>
  </v-card>
   ...
   <!-- SHOWING THE THUMBNAIL LIST -->
    ...
    font-size: 25px;">{{thumb.size}}</v-card-subtitle>
    <v-card-actions>
      <v-spacer></v-spacer>
      <v-btn :href="thumb.base64" :download="thumb.name" style=" text-decoration: none;"  :disabled="thumb.base64.length ==''" class="ma-2" tile outlined color="primary" >Download</v-btn>
      <v-btn @click="previewThumbNail(thumb)" class="ma-2" tile outlined color="primary">Preview
    </v-btn>
    <v-spacer></v-spacer>
    </v-card-actions>
  </v-card>
...
methods:{
  getYoutubeData(){
   let isValid = this.validateYouTubeUrl(this.youtubeUrl);
        if (isValid) {
          ...
          this.allFormateVideo=[{name:'11'},{name:'22'},{name:'33'},{name:'44'}]
          }
  ...
  showThumbNail(){
    this.allFormateThumb = [
            { name: "maxresdefault",  base64:'data',size: "HD 1920x1080",url: `https://i.ytimg.com/vi/${this.videoId}/maxresdefault.jpg`},
              { name: "sddefault",  base64:'data',size: "SD 640x480",url: `https://i.ytimg.com/vi/${this.videoId}/sddefault.jpg` },
              { name: "hqdefault",  base64:'data',size: "HQ 480x360",url: `https://i.ytimg.com/vi/${this.videoId}/hqdefault.jpg` },
              { name: "mqdefault", base64:'data', size: "MQ 320x180",url: `https://i.ytimg.com/vi/${this.videoId}/mqdefault.jpg` },
          ];
      ...
      xhr.onload = function() {
        if (xhr.readyState === 4) {
          if (xhr.status === 200) {
            ...
            _this.snackbarText = _this.allFormateThumb[i].size + " Image Quality Not Found";
          }
        }
      };
